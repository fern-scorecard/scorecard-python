...
from tracer.database import LogRecord, Session, TestCase, TestSet
...
sessions: ContextVar[Dict[int, "ScorecardSessionManager"]] = ContextVar(
    "sessions", default={}
)
...
class RunContextManager:
    def __init__(
        self, test_set: TestSet | int, runnable: Callable[..., Any] | None = None
    ) -> None:
        ...
        self.session = Session(
            tags={"type": "test_run", "source_test_set": str(self.test_set.id)}
        ).insert()
        ...
        if self.runnable:
            for input in self:
                try:
                    ...
                    raise ValueError(
                        "The supplied test case does not match the given function"
                    ) from e
        else:
            raise ValueError(
                "You must provide a runnable function if you want to run the test set."
            )
...
class ScorecardSessionManager:
    def __init__(
        ...
        if create_test_set:
            if not test_set_prefix:
                raise ValueError(
                    "You must provide a test set name if you want to create a test set."
                )
    ...
    def add_test_case(
        ...
        if key not in self.test_set_map.keys():
            test_set = TestSet(
                name=f"{self.test_set_prefix}#{function_name}#{'intermediate' if intermediate else 'root'}",
...
def enforce_full_type_annotations(func: Callable[..., Any], sig: Signature) -> None:
    ...
    if sig.return_annotation is _empty:
        raise TypeError(f"Function '{func.__name__}': Return type is unannotated")


def type_check(
    func: Callable[..., Any], args: Tuple, kwargs: Dict, sig: Signature
) -> BoundArguments:
...
def scorecard_function_tracer(func):
    ...
    def wrapper(*args, **kwargs):
        if current_session_id.get() == -1 and not is_run_context.get():
            raise ValueError(
                "No session found. You must use the SessionManager context manager."
            )
        ...
        if parent_span_id == "":  # this is a root span
            ...
            if (
                not is_run_context.get()
                and session_manager
                and session_manager.create_test_set
            ):
            ...
            if (
                ...
                and session_manager
                and session_manager.create_test_set
                and session_manager.include_intermediates
            ):
                session_manager.add_test_case(
                    input, output, func.__name__, sig, intermediate=True
                )
